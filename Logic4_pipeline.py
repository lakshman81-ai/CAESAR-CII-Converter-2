...
def build_hybrid_cii(
    ...
    nb  = len(bend_rows)  if bend_rows  else _count_from_aux(aux_data, "BEND", 3)
    nrg = len(rigid_rows) if rigid_rows else _count_from_aux(aux_data, "RIGID", 3)
    nr  = len(rest_rows)  if rest_rows  else _count_from_aux(aux_data, "RESTRANT", 4)
    ns  = len(sif_rows)   if sif_rows   else _count_from_aux(aux_data, "SIF&TEES", 2)
    ...
    ctrl["aux_counts"] = [
        ...
        0,    # NUMEXP
        nr,   # NUMRES
        0, 0, 0, 0, 0, 0,  # DISP,FORC,UNI,WIND,OFF,ALLOW
        ns,   # NUMSIF
        0, 0  # REDUCERS, FLANGES
    ]
