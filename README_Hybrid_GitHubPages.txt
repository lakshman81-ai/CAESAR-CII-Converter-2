...
2. Local ACCDB Mode: MS Access binary `.accdb` files cannot be processed via WebAssembly. If a user uploads an `.accdb` file, the webpage will automatically forward the request to `http://127.0.0.1:8000/convert_accdb`. The user must be running `python3 app.py` on their local machine for this to succeed.
